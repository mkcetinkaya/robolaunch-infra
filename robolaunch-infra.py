...
terraform_init_folder = Path("terraform/.terraform/")
...
with open('terraform/tf.json', 'w', encoding='utf-8') as f:
...
while True:
  ...
  for i in ret.items:
    if i.status.phase == "Running":
      ...
      j=1
      for i in ret_node.items:
        # Read in the file
        with open('rook/cluster.yaml', 'r') as file :
          filedata = file.read()
